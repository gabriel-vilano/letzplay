...
describe("findExposedSecrets", () => {
  it("aponta secret key do Supabase em variável pública", () => {
    expect(
      findExposedSecrets({ NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: FAKE_SECRET_KEY })
    ).toEqual([
      {
  ...
  it("aponta connection string do Postgres com senha em variável pública", () => {
    const exposed = findExposedSecrets({ NEXT_PUBLIC_DATABASE_URL: FAKE_POSTGRES_URL });
  ...
  it("ignora segredos em variáveis server-only (sem NEXT_PUBLIC_)", () => {
    expect(
      findExposedSecrets({
        ...
        POSTGRES_URL: FAKE_POSTGRES_URL,
      })
    ).toEqual([]);
  });
  ...
  it("nunca devolve o valor do segredo", () => {
    const serialized = JSON.stringify(
      findExposedSecrets({ NEXT_PUBLIC_QUALQUER: FAKE_SECRET_KEY })
    );
